...
function classifySphere(vCenter, vNormal, vPoint, iRadius) {
  var iDistance =
    ...
    vNormal.getY() * vCenter.getY() +
    planeDistance(vNormal, vPoint);
...
function collideEdgeWithEdge(oEdge1, oEdge2, oPointInt) {
  ...
  if (oPointInt) {
    oPointInt.set(
      pA.getX() + s * (pB.getX() - pA.getX()),
      pA.getY() + s * (pB.getY() - pA.getY())
    );
  }
  if (0 <= s && s <= 1 && 0 <= t && t <= 1) {
    return true;
...
function intersect2D_Segments(S1, S2, I0, I1) {
  ...
  if (Math.abs(D) < 0.00000001) {
    ...
    if (du == 0 && dv == 0) {
      ...
      if (S1.m_pA.getX() != S2.m_pA.getX() || S1.m_pA.getY() != S2.m_pA.getY())
...
function inSegment(P, S) {
  if (S.m_pA.getX() != S.m_pB.getX()) {
    // S is not vertical
    if (S.m_pA.getX() <= P.getX() && P.getX() <= S.m_pB.getX()) return 1;
    if (S.m_pA.getX() >= P.getX() && P.getX() >= S.m_pB.getX()) return 1;
  } else {
    // S is vertical, so test y coordinate
    if (S.m_pA.getY() <= P.getY() && P.getY() <= S.m_pB.getY()) return 1;
    if (S.m_pA.getY() >= P.getY() && P.getY() >= S.m_pB.getY()) return 1;
  }
  return 0;
}
...
module.exports = {
  ...
  angleBetweenVectors,
  HALF_PI,
  reflectVectorV2,rotateVector2D,
  toRadian,
  reflectVectorV2,
  closestPointOnLine
};
